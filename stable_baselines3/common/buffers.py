...
class RolloutRNDBuffer(BaseBuffer):
    """
    Rollout buffer used in on-policy algorithms like A2C/PPO.
    It corresponds to ``buffer_size`` transitions collected
    using the current policy.
    ...
    def __init__(
        ...
        self.observations, self.actions, self.rewards, self.advantages = None, None, None, None
        self.returns, self.episode_starts, self.values, self.log_probs = None, None, None, None
        self.int_rewards, self.int_advantages, self.int_values, self.int_returns = None, None, None, None
        self.generator_ready = False
        self.reset()
    ...
    def reset(self) -> None:
        ...
        self.actions = np.zeros((self.buffer_size, self.n_envs, self.action_dim), dtype=np.float32)
        self.rewards = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.int_rewards = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.returns = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.episode_starts = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.values = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor, last_int_values: th.Tensor, dones: np.ndarray) -> None:
        """
        Post-processing step: compute the lambda-return (TD(lambda) estimate)
        and GAE(lambda) advantage.
        ...
        # bonus are not episodic according to rnd
        last_gae_lam = 0
        for step in reversed(range(self.buffer_size)):
            if step == self.buffer_size - 1:
            ...
            delta = self.int_rewards[step] + self.int_gamma * next_int_values * next_non_terminal - self.int_values[step]
            last_gae_lam = delta + self.int_gamma * self.gae_lambda * next_non_terminal * last_gae_lam
            self.int_advantages[step] = last_gae_lam
        # TD(lambda) estimator, see Github PR #375 or "Telescoping in TD(lambda)"
        # in David Silver Lecture 4: https://www.youtube.com/watch?v=PnHCvfgC_ZA
        self.returns = self.advantages + self.values
    ...
    def add(
        ...
        # print(self.observations.shape, "shape of observation buffer")
        # print(obs.shape, "shape of newly collected observation")

        self.observations[self.pos] = np.array(obs).copy()
        self.next_observations[self.pos] = np.array(next_obs).copy()
        self.actions[self.pos] = np.array(action).copy()
        self.rewards[self.pos] = np.array(reward).copy()
        self.int_rewards[self.pos] = int_reward.clone().cpu().numpy().flatten()
        self.episode_starts[self.pos] = np.array(episode_start).copy()
        self.values[self.pos] = value.clone().cpu().numpy().flatten()
    ...
    def _get_samples(
        ...
        batch_inds: np.ndarray,
        env: Optional[VecNormalize] = None,
    ) -> RolloutRNDBufferSamples:  # type: ignore[signature-mismatch] #FIXME
        data = (
            self.observations[batch_inds],
        ...
        return RolloutRNDBufferSamples(*tuple(map(self.to_torch, data)))
